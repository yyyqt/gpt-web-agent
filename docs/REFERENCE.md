...
First-time setup: [Chinese illustrated guide](GETTING-STARTED.zh-CN.md).
...
the [Chinese setup guide](GETTING-STARTED.zh-CN.md) for account steps.
...
Connect through [the ChatGPT setup guide](CHATGPT.md). For any stdio MCP
...
Keep the host awake and online. See [background operation](BACKGROUND.md).
...
See [protocols, limits and image handoff caveats](IMAGES-AND-EDITS.md).
...
A separate authenticated option is [under design](PUBLIC-CONNECTION-DESIGN.md). The current localhost HTTP endpoint must not be forwarded to the public internet.
